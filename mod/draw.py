...
def draw_path_edit_overlay(screen, segment_idx, nodes, font_obj):
    ...
    if segment_idx is not None and segment_idx < len(nodes) - 1:
        path_data = nodes[segment_idx].get("path_to_next", {}) or {}
        spline_raw = str(path_data.get("spline_type", "uniform")).strip().lower()
        spline_mode = "Centripetal" if spline_raw in ("centripetal", "cent", "c") else "Uniform"
        help_text = [
            "PATH EDIT MODE",
            ...
            f"Spline: {spline_mode} (U toggles)",
            "P or ESC: Exit"
        ]
    ...
    for i in range(len(nodes) - 1):
        ...
        if path_data.get("use_path", False):
            ...
            if len(control_points) >= 2:
                ...
                curve_points = generate_bezier_path(cps, num_samples=50, spline_type=spline_type)
...
def draw_path_edit_mode(surface, nodes, segment_idx, dragging_idx, control_radius):
    ...
    if not control_points:
        return
    spline_type = path_data.get("spline_type")
    curve_points = generate_bezier_path(control_points, num_samples=50, spline_type=spline_type)
    if len(curve_points) > 1:
...
def get_field_object_entries(cfg):
    """Return list of field objects with ids, kinds, labels, and polygons."""
    cached = _FIELD_POLY_CACHE.get("entries")
    ...
    for x in (1.0, 5.0):
        ...
        side = "left" if x < 3.0 else "right"
        entries.append({
            "id": f"long_goal_{side}",
            "kind": "long_goal",
            "label": f"Long Goal ({side})",
            "poly": _oriented_rect_poly(c, 90.0, THICK, LONG_LEN)
        })
        end_offset_px = (LONG_LEN * 0.5 - THICK * 0.5) * PPI
        for end_tag, end_sgn in (("top", 1.0), ("bottom", -1.0)):
            by = c[1] - (end_sgn * end_offset_px)
            entries.append({
                "id": f"long_goal_brace_{side}_{end_tag}",
                "kind": "long_goal_brace",
                "label": f"Long Goal Brace ({side}, {end_tag})",
                "poly": _oriented_rect_poly((c[0], by), 90.0, THICK, THICK)
            })
    c = _tile_center_px(3.0, 3.0)
    entries.append({
        "id": "center_goal_pos45",
        "kind": "center_goal",
        "label": "Center Goal (+45)",
        "poly": _oriented_rect_poly(c, 45.0, THICK, CENTER_LEN)
    })
    entries.append({
        "id": "center_goal_neg45",
        "kind": "center_goal",
        "label": "Center Goal (-45)",
        "poly": _oriented_rect_poly(c, -45.0, THICK, CENTER_LEN)
    })
    ...
    for x in (1.0, 5.0):
        ...
        entries.append({
            "id": f"matchloader_{side}_bottom",
            "kind": "matchloader",
            "label": f"Matchloader ({side}, bottom)",
            "poly": _oriented_rect_poly(cb, 90.0, ML_SIZE, ML_SIZE)
        })
        ct = _tile_center_px(x, 6.0)
        ct = (ct[0], ct[1] + inset_in * PPI)
        entries.append({
            "id": f"matchloader_{side}_top",
            "kind": "matchloader",
            "label": f"Matchloader ({side}, top)",
            "poly": _oriented_rect_poly(ct, 90.0, ML_SIZE, ML_SIZE)
        })
    ...
    entries.append({
        "id": "park_zone_bottom",
        "kind": "park_zone",
        "label": "Park Zone (bottom)",
        "poly": [(x0, WINDOW_HEIGHT - d_px), (x1, WINDOW_HEIGHT - d_px),
    ...
    entries.append({
        ...
        "kind": "park_zone",
        "label": "Park Zone (top)",
        "poly": [(x0, 0), (x1, 0), (x1, d_px), (x0, d_px)]
    })
    _FIELD_POLY_CACHE["entries"] = entries
    return entries

def get_field_object_polys(cfg):
    """Return list of (kind, polygon) for field objects."""
    cached = _FIELD_POLY_CACHE.get("polys")
    if cached is not None:
        return cached
    polys = [(entry["kind"], entry["poly"]) for entry in get_field_object_entries(cfg)]
    _FIELD_POLY_CACHE["polys"] = polys
    return polys

def _mcl_sim_enabled(cfg):
    """Handle mcl sim enabled."""
    try:
        mcl_cfg = cfg.get("mcl", {}) if isinstance(cfg, dict) else {}
        enabled = mcl_cfg.get("enabled", 0) if isinstance(mcl_cfg, dict) else 0
...
def draw_field_objects(surface, cfg):
    ...
    try:
        color_map = {
            ...
            'long_goal_brace': (255, 150, 40),
            'center_goal': (70, 140, 255),
            'park_zone': (220, 60, 60),
            'matchloader': (245, 210, 60)
        }
        show_braces = _mcl_sim_enabled(cfg)
...
def draw_geometry_borders(surface, nodes, cfg, init_heading):
    ...
    for i, node in enumerate(nodes):
        ...
        if node_offset_in != 0.0 and nodes[i - 1].get("move_to_pose"):
            pose_h = nodes[i - 1].get("pose_heading_deg")
            if pose_h is None:
                pose_h = heading_from_points(prev_start if prev_start is not None else prev_pos, p)
            try:
                pose_h = float(pose_h)
            except Exception:
                pose_h = heading_from_points(prev_start if prev_start is not None else prev_pos, p)
    ...
    def _incoming_path_heading(idx: int):
        ...
        if path_pts and len(path_pts) >= 2:
            ...
            spline_type = pd.get("spline_type")
            pts = generate_bezier_path(cps, num_samples=50, spline_type=spline_type)
        if not pts:
            return None
    ...
    for i, node in enumerate(nodes):
            ...
            if outgoing_h is None and (next_pd.get("use_path", False) or next_pd.get("pose_preview_points")):
                ...
                if path_pts and len(path_pts) >= 2:
                    ...
                    if len(cps) >= 2:
                        ...
                        spline_type = next_pd.get("spline_type")
                        pts = generate_bezier_path(cps, num_samples=20, spline_type=spline_type)
                        if pts:
        ...
        if node.get("reverse", False) and not has_reverse_act:
            reverse_state_local = not reverse_state_local
        for act in acts:
            if act.get("type") == "reverse":
                ...
                if state is None:
                    ...
                    reverse_state_local = bool(state)
        outgoing_reverse = reverse_state_local
        eff_heading = (base_h + (180.0 if outgoing_reverse else 0.0)) % 360.0
        reverse_state = reverse_state_local
        if i > 0:
